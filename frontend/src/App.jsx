...
function LifeOSPage() {
  ...
  // نماذج الإضافة
  const [finForm, setFinForm] = useState({ type: 'expense', amount: '', category: 'طعام', note: '' });
  ...
  const [relForm, setRelForm] = useState({ person_name: '', relation_type: 'friend' });
  ...
  const tabs = [
    { id: 'overview', icon: '◈', label: lang === 'ar' ? 'نظرة شاملة' : 'Overview' },
    ...
    { id: 'wellness', icon: '🧘', label: lang === 'ar' ? 'الصحة' : 'Wellness' },
  ];

  if (!loaded) return <p style={{ color: 'var(--muted)' }}>⏳ {lang === 'ar' ? 'جاري تحميل حياتك...' : 'Loading your life...'}</p>;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
      ...
      <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
        {tabs.map(tb => (
          <button key={tb.id} onClick={() => setTab(tb.id)} style={{
            padding: '9px 18px', borderRadius: '3px', border: `1px solid ${tab === tb.id ? 'var(--cyan)' : 'rgba(0,229,255,0.15)'}`,
      ...
      {tab === 'overview' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '1rem' }}>
          <Gauge value={finance.summary.balance} max={Math.max(finance.summary.income, 1)} label={lang === 'ar' ? 'الرصيد' : 'Balance'} color={finance.summary.balance >= 0 ? '#00e676' : '#ff2d78'} suffix=" $" />
          <Gauge value={Math.round(study.total_minutes / 60)} max={40} label={lang === 'ar' ? 'ساعات الدراسة' : 'Study Hours'} color="#00e5ff" suffix={hs} />
          <Gauge value={home.tasks.filter(t => t.status !== 'done').length} max={20} label={lang === 'ar' ? 'مهام البيت' : 'Home Tasks'} color="#f59e0b" />
          <Gauge value={people.people.length} max={20} label={lang === 'ar' ? 'العلاقات' : 'People'} color="#7c4dff" />
          <Gauge value={wellness.logs[0]?.mood || 5} max={10} label={lang === 'ar' ? 'المزاج' : 'Mood'} color="#00e676" suffix="/10" />
        </div>
      )}

      {/* 💰 المال */}
      {tab === 'finance' && (
        <div className="glass-panel hud-panel" style={{ padding: '1.3rem' }}>
          <h3 style={{ margin: '0 0 1rem', color: 'var(--text)' }}>💰 {lang === 'ar' ? 'المالية' : 'Finance'}</h3>
          <div style={{ display: 'flex', gap: '1.5rem', marginBottom: '1rem', flexWrap: 'wrap', fontSize: '0.9rem' }}>
            <span>{lang === 'ar' ? 'الدخل:' : 'Income:'} <strong style={{ color: '#00e676' }}>${finance.summary.income}</strong></span>
            <span>{lang === 'ar' ? 'المصاريف:' : 'Expenses:'} <strong style={{ color: '#ff2d78' }}>${finance.summary.expense}</strong></span>
            <span>{lang === 'ar' ? 'الرصيد:' : 'Balance:'} <strong style={{ color: finance.summary.balance >= 0 ? '#00e676' : '#ff2d78' }}>${finance.summary.balance}</strong></span>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '0.8rem', marginBottom: '1rem' }}>
            <select value={finForm.type} onChange={e => setFinForm({ ...finForm, type: e.target.value })} style={fieldStyle}>
              ...
              <option value="income">{lang === 'ar' ? 'دخل' : 'Income'}</option>
            </select>
            <input type="number" placeholder={lang === 'ar' ? 'المبلغ' : 'Amount'} value={finForm.amount} onChange={e => setFinForm({ ...finForm, amount: e.target.value })} style={fieldStyle} />
            <input placeholder={lang === 'ar' ? 'الفئة (طعام، مواصلات...)' : 'Category'} value={finForm.category} onChange={e => setFinForm({ ...finForm, category: e.target.value })} style={fieldStyle} />
            <button onClick={() => { if (finForm.amount > 0) post('/api/finance', finForm); }} style={{ padding: '10px', background: 'linear-gradient(90deg,#0077ff,#00e5ff)', color: '#001018', border: 'none', borderRadius: '3px', fontWeight: 800, cursor: 'pointer' }}>➕</button>
          </div>
      ...
      {tab === 'study' && (
        <div className="glass-panel hud-panel" style={{ padding: '1.3rem' }}>
          <h3 style={{ margin: '0 0 1rem', color: 'var(--text)' }}>📚 {lang === 'ar' ? 'الدراسة' : 'Study'}</h3>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '0.8rem', marginBottom: '1rem' }}>
            <input placeholder={lang === 'ar' ? 'المادة' : 'Subject'} value={studyForm.subject} onChange={e => setStudyForm({ ...studyForm, subject: e.target.value })} style={fieldStyle} />
            <input type="number" placeholder={lang === 'ar' ? 'الدقائق' : 'Minutes'} value={studyForm.duration_minutes} onChange={e => setStudyForm({ ...studyForm, duration_minutes: e.target.value })} style={fieldStyle} />
            <select value={studyForm.quality} onChange={e => setStudyForm({ ...studyForm, quality: e.target.value })} style={fieldStyle}>
              <option value="high">{lang === 'ar' ? 'تركيز عالي' : 'High focus'}</option>
              <option value="medium">{lang === 'ar' ? 'متوسط' : 'Medium'}</option>
              <option value="low">{lang === 'ar' ? 'خفيف' : 'Light'}</option>
            </select>
            <button onClick={() => { if (studyForm.subject.trim()) post('/api/study', studyForm); }} style={{ padding: '10px', background: 'linear-gradient(90deg,#0077ff,#00e5ff)', color: '#001018', border: 'none', borderRadius: '3px', fontWeight: 800, cursor: 'pointer' }}>➕</button>
          </div>
          <p style={{ color: 'var(--muted)', fontSize: '0.85rem' }}>{lang === 'ar' ? 'إجمالي الدراسة:' : 'Total study:'} <strong style={{ color: '#00e5ff' }}>{Math.round(study.total_minutes / 60)}{hs} {Math.round(study.total_minutes % 60)}m</strong></p>
          {study.sessions.slice(0, 8).map(s => (
            <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '0.5rem 0', borderBottom: '1px solid rgba(0,229,255,0.08)', fontSize: '0.85rem' }}>
              <span style={{ color: 'var(--text)' }}>{s.subject} {s.topic ? `— ${s.topic}` : ''}</span>
      ...
      {tab === 'home' && (
        <div className="glass-panel hud-panel" style={{ padding: '1.3rem' }}>
          <h3 style={{ margin: '0 0 1rem', color: 'var(--text)' }}>🏠 {lang === 'ar' ? 'مهام البيت' : 'Home Tasks'}</h3>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '0.8rem', marginBottom: '1rem' }}>
            <input placeholder={lang === 'ar' ? 'المهمة' : 'Task'} value={homeForm.title} onChange={e => setHomeForm({ ...homeForm, title: e.target.value })} style={fieldStyle} />
            <input placeholder={lang === 'ar' ? 'الغرفة' : 'Room'} value={homeForm.room} onChange={e => setHomeForm({ ...homeForm, room: e.target.value })} style={fieldStyle} />
            <select value={homeForm.priority} onChange={e => setHomeForm({ ...homeForm, priority: e.target.value })} style={fieldStyle}>
            ...
            <button onClick={() => { if (homeForm.title.trim()) post('/api/home', homeForm); }} style={{ padding: '10px', background: 'linear-gradient(90deg,#0077ff,#00e5ff)', color: '#001018', border: 'none', borderRadius: '3px', fontWeight: 800, cursor: 'pointer' }}>➕</button>
          </div>
          {home.tasks.map(tk => (
            <div key={tk.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.6rem 0', borderBottom: '1px solid rgba(0,229,255,0.08)' }}>
              <span style={{ color: tk.status === 'done' ? 'var(--muted)' : 'var(--text)', textDecoration: tk.status === 'done' ? 'line-through' : 'none', fontSize: '0.9rem' }}>
                {tk.priority === 'urgent' ? '🔴' : tk.priority === 'high' ? '🟠' : '🔵'} {tk.title} {tk.room ? `(${tk.room})` : ''}
              </span>
              <button onClick={async () => {
                const headers = await getAuthHeaders();
                await fetch(`${API_URL}/api/home/${tk.id}`, { method: 'PATCH', headers, body: JSON.stringify({ status: tk.status === 'done' ? 'pending' : 'done' }) });
                loadAll();
      ...
      {tab === 'relations' && (
        <div className="glass-panel hud-panel" style={{ padding: '1.3rem' }}>
          <h3 style={{ margin: '0 0 1rem', color: 'var(--text)' }}>🤝 {lang === 'ar' ? 'العلاقات' : 'Relationships'}</h3>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '0.8rem', marginBottom: '1rem' }}>
            <input placeholder={lang === 'ar' ? 'الاسم' : 'Name'} value={relForm.person_name} onChange={e => setRelForm({ ...relForm, person_name: e.target.value })} style={fieldStyle} />
            <select value={relForm.relation_type} onChange={e => setRelForm({ ...relForm, relation_type: e.target.value })} style={fieldStyle}>
              ...
              <option value="friend">{lang === 'ar' ? 'صديق' : 'Friend'}</option>
              <option value="colleague">{lang === 'ar' ? 'زميل' : 'Colleague'}</option>
            </select>
            <button onClick={() => { if (relForm.person_name.trim()) post('/api/relationships', relForm); }} style={{ padding: '10px', background: 'linear-gradient(90deg,#0077ff,#00e5ff)', color: '#001018', border: 'none', borderRadius: '3px', fontWeight: 800, cursor: 'pointer' }}>➕</button>
          </div>
          {people.people.map(p => (
            <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '0.6rem 0', borderBottom: '1px solid rgba(0,229,255,0.08)', fontSize: '0.9rem' }}>
              ...
              <span style={{ color: 'var(--muted)', fontSize: '0.75rem' }}>{lang === 'ar' ? 'كل' : 'every'} {p.contact_frequency_days} {lang === 'ar' ? 'يوم' : 'days'}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {tab === 'wellness' && (
        <div className="glass-panel hud-panel" style={{ padding: '1.3rem' }}>
          <h3 style={{ margin: '0 0 1rem', color: 'var(--text)' }}>🧘 {lang === 'ar' ? 'الصحة والمزاج' : 'Wellness'}</h3>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '0.8rem', marginBottom: '1rem' }}>
            <div><label style={{ ...labelStyle, fontSize: '0.75rem' }}>{lang === 'ar' ? 'المزاج' : 'Mood'}: {wellForm.mood}/10</label>
            ...
            <div><label style={{ ...labelStyle, fontSize: '0.75rem' }}>{lang === 'ar' ? 'الطاقة' : 'Energy'}: {wellForm.energy}/10</label>
            ...
            <div><label style={{ ...labelStyle, fontSize: '0.75rem' }}>{lang === 'ar' ? 'النوم (ساعات)' : 'Sleep (hrs)'}</label>
            ...
            <button onClick={() => post('/api/wellness', wellForm)} style={{ padding: '10px', background: 'linear-gradient(90deg,#0077ff,#00e5ff)', color: '#001018', border: 'none', borderRadius: '3px', fontWeight: 800, cursor: 'pointer', alignSelf: 'end' }}>📝 {lang === 'ar' ? 'سجل' : 'Log'}</button>
          </div>
          {wellness.logs.slice(0, 7).map(w => (
            <div key={w.id} style={{ display: 'flex', gap: '1rem', padding: '0.5rem 0', borderBottom: '1px solid rgba(0,229,255,0.08)', fontSize: '0.8rem', color: 'var(--muted)' }}>
              ...
              <span style={{ marginLeft: 'auto' }}>{new Date(w.log_date).toLocaleDateString(lang === 'ar' ? 'ar-SA' : 'en')}</span>
            </div>
          ))}
        </div>
      )}

...
/* ========== الوكيل المنفذ (Function Calling) ========== */
const AGENT_TOOLS = [
  ...
  { name: 'add_expense', description: 'Record a financial expense', parameters: { type: 'OBJECT', properties: { amount: { type: 'NUMBER' }, category: { type: 'STRING' } }, required: ['amount'] } },
  { name: 'add_home_task', description: 'Add a home task', parameters: { type: 'OBJECT', properties: { title: { type: 'STRING' } }, required: ['title'] } },
];

async function fetchLifeData(userId) {
try {
const [fin, stu, home, rel, well] = await Promise.all([
supabase.from('finance_entries').select('type, amount').eq('user_id', userId).limit(100),
supabase.from('study_sessions').select('duration_minutes').eq('user_id', userId).limit(50),
supabase.from('home_tasks').select('title, status').eq('user_id', userId).limit(50),
supabase.from('relationships').select('person_name, last_contact, contact_frequency_days').eq('user_id', userId).limit(50),
supabase.from('wellness_logs').select('mood, energy, sleep_hours').eq('user_id', userId).order('log_date', { ascending: false }).limit(1),
]);
const finList = fin.data || [];
const income = finList.filter((e) => e.type === 'income').reduce((s, e) => s + Number(e.amount), 0);
const expense = finList.filter((e) => e.type === 'expense').reduce((s, e) => s + Number(e.amount), 0);
return {
finance: { income, expense, balance: income - expense },
study_minutes: (stu.data || []).reduce((s, e) => s + Number(e.duration_minutes), 0),
pending_home_tasks: (home.data || []).filter((t) => t.status !== 'done').length,
relationships: (rel.data || []).map((p) => p.person_name),
neglected_relationships: (rel.data || []).filter((p) => !p.last_contact || (Date.now() - new Date(p.last_contact).getTime()) / 86400000 > (p.contact_frequency_days || 7)).map((p) => p.person_name),
latest_wellness: (well.data || [])[0] || null,
};
} catch (e) { return null; }
}
const LIFE_TOOLS = [
{ name: 'add_expense', description: 'Record money movement (expense or income)', parameters: { type: 'OBJECT', properties: { amount: { type: 'NUMBER' }, kind: { type: 'STRING', description: 'expense|income' }, category: { type: 'STRING' } }, required: ['amount'] } },
  ...
  try {
    if (name === 'add_commitment') {
      const { data, error } = await supabase.from('commitments').insert({
        user_id: userId, title: String(args.title || 'التزام جديد'),
    ...
    if (name === 'add_expense') {
      ...
      if (error) throw error;
      return `recorded expense ${data.amount} (${data.category})`;
    }
    if (name === 'add_home_task') {
      const { data, error } = await supabase.from('home_tasks').insert({ user_id: userId, title: String(args.title || 'مهمة') }).select().single();
      if (error) throw error;
      return `added home task "${data.title}"`;
    }

if (name === 'add_expense') {
const { data, error } = await supabase.from('finance_entries').insert({
user_id: userId, type: args.kind === 'income' ? 'income' : 'expense',
amount: Number(args.amount || 0), category: String(args.category || 'عام'),
}).select().single();
if (error) throw error;
...
app.post('/api/agent', requireAuth, async (req, res) => {
  try {
    ...
    const system = (lang === 'en'
      ? `You are AQL, the user's JARVIS-style life OS agent. Address him as "sir". Execute his request using tools when needed, then reply in 2-4 sentences. `
      : `أنت "عَقْل"، وكيل نظام الحياة بأسلوب جارفس. خاطبه بلقب "سيدي". نفّذ طلبه باستخدام الأدوات عند الحاجة ثم أرد بجملتين إلى أربع. `)
      + `Current commitments: ${JSON.stringify(list)}`;
    const contents = [{ role: 'user', parts: [{ text: message }] }];
    const actions = [];
    let reply = '';
    for (let step = 0; step < 5; step++) {
      const data = await agentGemini({
        system_instruction: { parts: [{ text: system }] },
        contents,
        tools: [{ function_declarations: [...AGENT_TOOLS, ...LIFE_TOOLS] }]
      });
...
if (response.status === 503 || response.status === 429 || response.status === 404) { lastError = new Error(`Gemini ${response.status} on ${model}`); continue; }
if (!response.ok) { const errText = await response.text(); throw new Error(`Gemini error ${response.status}: ${errText}`); }
return await response.json();
} catch (e) { lastError = e; }
}
throw lastError || new Error('All Gemini models failed');
...
const life = await fetchLifeData(req.user.id);
const system = (lang === 'en'
? 'You are AQL, a JARVIS-style life OS agent. Address the user as "sir". Execute the request using the provided tools when needed, then reply in 2-4 sentences. '
: 'أنت "عَقْل"، وكيل نظام الحياة بأسلوب جارفس. خاطبه بلقب "سيدي". نفّذ طلبه باستخدام الأدوات عند الحاجة ثم أرد بجملتين إلى أربع. ')

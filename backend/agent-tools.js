...
async function wikipediaSearch(query, lang = 'ar') {
  ...
  return (data.query?.search || []).slice(0, 3).map((h) => ({
    source: 'Wikipedia', title: h.title,
...
async function gatherWebResults(query, lang = 'ar') {
  ...
  return [...(wiki.status === 'fulfilled' ? wiki.value : []), ...(duck.status === 'fulfilled' ? duck.value : [])].slice(0, 5);
}

export const toolsRegistry = [
  { declaration: { name: 'web_search', description: 'Search the live web for real-time information, news, people, topics. Use when the user asks to search or find something online.', parameters: { type: 'OBJECT', properties: { query: { type: 'STRING', description: 'what to search for' } }, required: ['query'] } },
  ...
  { declaration: { name: 'find_software', description: 'Find and recommend software, programs or apps for a purpose, with real links. Use when the user asks for a program or app.', parameters: { type: 'OBJECT', properties: { purpose: { type: 'STRING', description: 'what the program should do' }, platform: { type: 'STRING', description: 'windows|mac|android|ios|web|any' } }, required: ['purpose'] } },
    execute: async (userId, args) => {
  ...
  { declaration: { name: 'add_commitment', description: 'Add a new weekly time commitment', parameters: { type: 'OBJECT', properties: { title: { type: 'STRING' }, hours_per_week: { type: 'NUMBER' }, type: { type: 'STRING', description: 'study|work|health|personal|sleep' }, intensity: { type: 'STRING', description: 'low|medium|high' }, time_slot: { type: 'STRING', description: 'morning|afternoon|evening|late_night|mixed' } }, required: ['title', 'hours_per_week'] } },
  ...
  { declaration: { name: 'reduce_hours', description: 'Reduce weekly hours of an existing commitment (match by title)', parameters: { type: 'OBJECT', properties: { title: { type: 'STRING' }, new_hours: { type: 'NUMBER' } }, required: ['title', 'new_hours'] } },
    execute: async (userId, args, { supabase }) => {
  ...
  { declaration: { name: 'archive_commitment', description: 'Archive/remove an existing commitment (match by title)', parameters: { type: 'OBJECT', properties: { title: { type: 'STRING' } }, required: ['title'] } },
    execute: async (userId, args, { supabase }) => {
      const { data: row } = await supabase.from('commitments').select('*').eq('user_id', userId).eq('status', 'active').ilike('title', `%${String(args.title || '')}%`).maybeSingle();
      if (!row) return `commitment "${args.title}" not found`;
  ...
  { declaration: { name: 'create_goal', description: 'Create a strategic goal', parameters: { type: 'OBJECT', properties: { title: { type: 'STRING' } }, required: ['title'] } },
    execute: async (userId, args, { supabase }) => {
  ...
  { declaration: { name: 'add_expense', description: 'Record a financial expense or income', parameters: { type: 'OBJECT', properties: { amount: { type: 'NUMBER' }, kind: { type: 'STRING', description: 'expense|income' }, category: { type: 'STRING' } }, required: ['amount'] } },
    execute: async (userId, args, { supabase }) => {
  ...
  { declaration: { name: 'add_home_task', description: 'Add a home task', parameters: { type: 'OBJECT', properties: { title: { type: 'STRING' }, priority: { type: 'STRING', description: 'low|medium|high|urgent' } }, required: ['title'] } },
    execute: async (userId, args, { supabase }) => {
  ...
  { declaration: { name: 'log_study', description: 'Log a study session', parameters: { type: 'OBJECT', properties: { subject: { type: 'STRING' }, duration_minutes: { type: 'NUMBER' } }, required: ['subject', 'duration_minutes'] } },
    execute: async (userId, args, { supabase }) => {
  ...
  { declaration: { name: 'mark_contact', description: 'Mark that the user contacted a person today', parameters: { type: 'OBJECT', properties: { person_name: { type: 'STRING' } }, required: ['person_name'] } },
    execute: async (userId, args, { supabase }) => {
...
export const getOpenAITools = () => toolsRegistry.map((tool) => ({
  ...
  function: {
    ...
    parameters: { type: 'object', properties: tool.declaration.parameters.properties, required: tool.declaration.parameters.required || [] },
  },
}));
export const runAgentTool = async (userId, name, args, context) => {
  const tool = toolsRegistry.find((t) => t.declaration.name === name);
  if (!tool) return `unknown tool ${name}`;
  try { return await tool.execute(userId, args, context); }

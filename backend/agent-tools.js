...
/* تُستخدم داخل /api/chat: تبحث فقط إذا كانت الرسالة طلب بحث أو معرفة */
...
export const toolsRegistry = [
  /* ----- أدوات الحياة (تنفّذ في قاعدة البيانات) ----- */
  {
    declaration: {
      name: 'add_commitment',
  ...
  /* ----- أدوات العقل الكامل (معرفة حية من الويب) ----- */
  {
    declaration: {
      name: 'web_search',
      description: 'Search the live web for real-time information: news, facts, people, topics, definitions. Use whenever the user asks to search or asks about real-world knowledge.',
  ...
  },
  {
    declaration: {
      name: 'find_software',
      description: 'Find and recommend software, programs or apps for a purpose. Returns real descriptions and links. Use when the user asks for a program or app.',
      parameters: { type: 'OBJECT', properties: { purpose: { type: 'STRING', description: 'what the program should do, e.g. video editing, screen recording' }, platform: { type: 'STRING', description: 'windows|mac|android|ios|web|any' } }, required: ['purpose'] },
    },
    execute: async (userId, args) => {
      const purpose = String(args.purpose || '');
      const platform = String(args.platform || 'any');
      const q = `best free ${purpose} software for ${platform}`;
      const results = await gatherWebResults(q, 'en');
